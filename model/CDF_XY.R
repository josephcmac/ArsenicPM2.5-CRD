...
plot_combined_matrices <- function(observed_matrix, expected_matrix, file_name, title1, title2) {
  ...
  box() 
  axis(1, at = seq(0, 1, length.out = ncol(expected_matrix)), labels = FALSE) # Customize x-axis
  axis(2, at = seq(0, 1, length.out = nrow(expected_matrix)), labels = FALSE) # Customize y-axis
  
  dev.off()
}

combined_file <- paste0(DATASETS, "/images/CDF_XY/combined_CDF_XY-", n_clusters, "-", age, "-", ifelse(male, "male", "female"), ".png")

plot_combined_matrices(Observed, Expected, combined_file, "Observed CDF XY", "Expected CDF XY")
