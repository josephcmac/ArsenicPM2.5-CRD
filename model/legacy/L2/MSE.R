...
d <- create_table(df, alpha = 0.6, n_clusters = 2, error = mse) %>% 
	rename(mae2 = mae)

d_next <- create_table(df, alpha = 0.6, n_clusters = 3, error = mse) %>% 
	rename(mae3 = mae)

d <- merge(d, d_next)

d_next <- create_table(df, alpha = 0.6, n_clusters = 4, error = mse) %>% 
	rename(mae4 = mae)

d <- merge(d, d_next)

d_next <- create_table(df, alpha = 0.6, n_clusters = 5, error = mse) %>% 
	rename(mae5 = mae)

d <- merge(d, d_next)

d_next <- create_table(df, alpha = 0.6, n_clusters = 6, error = mse) %>% 
	rename(mae6 = mae)
...
CairoPNG(paste0(DATASETS,"/images/MSE/MSE-male.png"))
ggplot(d_long, aes(x = age_min, y = p_value, color = k)) +
  geom_point() +
  geom_line() +
  labs(title = "MSE par tranche d'âge (hommes)",
       x = "Tranche d'Âge",
       y = "MSE",
       color = "k") +
  theme_minimal()
...
CairoPNG(paste0(DATASETS,"/images/MSE/MSE-female.png"))
ggplot(d_long, aes(x = age_min, y = p_value, color = k)) +
  geom_point() +
  geom_line() +
  labs(title = "MSE par tranche d'âge (femmes)",
       x = "Tranche d'Âge",
       y = "MSE",
       color = "k") +
  theme_minimal()
