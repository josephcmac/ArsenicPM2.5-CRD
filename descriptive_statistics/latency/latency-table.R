...
u <- map_df(0:18, ~{
  ...
  y <- sapply(x, function(i) comp_p_Female(make_combination(df_env_yearly, 
          df_health %>% filter(age == .x) %>% select(-age), 
        i)))

  z <- data.frame(latency=x,p_value=y)
  z$age_min <- 5*.x
  z$age_max <- 5*.x+4
  z
...
u_wide <- pivot_wider(u, names_from = latency, values_from = p_value, names_prefix = "latency_")

latency_df <- data.frame( age_min=5*(0:18),  age_max=5*(0:18)+4, latency = sapply(1:19, function(i) (u_wide[i,][3:12] %>% as.numeric %>% which.min) ) ) %>%
	mutate(
	       age_min = as.integer(age_min), 
	       age_max = as.integer(age_max),
	       latency = as.integer(latency)
	)

print(xtable::xtable(latency_df), include.rownames=FALSE)
